...
fs = require 'fs-plus'
PathWatcher = require path.join(atom.packages.resourcePath,'node_modules/pathwatcher/')
File = require './file'
...
module.exports =
class Directory
  constructor: ({@name, fullPath, @symlink, @expansionState, @isRoot, @ignoredPatterns, @useSyncFS, @stats, @virDirectory}) ->
    # console.log "directory -----------------:", @name
    # console.log @name, fullPath, @symlink, @expansionState, @isRoot, @ignoredPatterns, @useSyncFS, @stats, @virDirectory
    ...
    @isRoot ?= false
    @isVirtual = @virDirectory?.isVirtual
    @isVirtual ?= false
    if @isVirtual
      if @virDirectory.basedir
        fullPath = path.join fullPath, @virDirectory.basedir
    else
      if atom.config.get('tree-view.squashDirectoryNames')
        console.log "tree squashDirectoryNames"
  ...
  subscribeToRepo: ->
    ...
    console.log "subscribeToRepo repo:",repo
    return unless repo?
  ...
  isVirPathIgnored:(filePath) ->
    # console.log "||||||||||||||||||||||||",@virDirectory,  @virDirectory?.pathIgnored?.indexOf(filePath) > -1
    if @virDirectory?.pathIgnored?.indexOf(filePath) > -1
      return true
  ...
  getEntries:->
    # console.log @virDirectory
    # console.log @virDirectory?.hasOwnProperty('directories')
    if @virDirectory?.hasOwnProperty('directories')
  ...
  getRealEntries:() ->
    # console.log "get real entrye ++++++++"
    names = @getNames()
    files = []
    directories = []
    for name in names
      fullPath = path.join(@path, name)
      continue if @isVirPathIgnored(fullPath)
      ...
      if stat.isDirectory?()
        if @entries.hasOwnProperty(name)
          ...
          virDirectory = {}
          oNewVirDirs = @virDirectory?.subdirs
          unless !( oNewVirDirs and (_.size(oNewVirDirs) > 0))
            virDirectory = @newVirDirsParams(oNewVirDirs)
          expansionState = @expansionState.entries[name]
          directories.push(new Directory({name, fullPath, symlink, expansionState, @ignoredPatterns, @useSyncFS, stats: statFlat, virDirectory}))
      else if stat.isFile?()
        if @entries.hasOwnProperty(name)
          # push a placeholder since this entry already exists but this helps
          # track the insertion index for the created views
  ...
  newVirDirsParams:(newVirDirs) ->
    {
      directories:newVirDirs
    }


  getVirtualEntries: ->
    # console.log "+++--- getVirtualEntries  ---+++", @virDirectory
    oVirTualDirs = @virDirectory.directories
    names = []
    if _.size(oVirTualDirs) > 0
      # if @virDirectory.isShowRealDir TODO
      names = _.keys(oVirTualDirs)
      # sTmpPath = @path
    else # if @virDirectory.isShowRealDir TODO
      try
        # if @virtualDirs?.length > 0
        # sTmpPath = path.join @path,@virDirectory.basedir
        names = fs.readdirSync(@path)
        console.log "names:",names
        names
      catch error
    ...
    for name in names
      fullPath = path.join(@path, name)
      continue if @isVirPathIgnored(name)
      # console.log "============== is virtual before" ,oTmpDirs
      oTmpDirs = oVirTualDirs[name]
      # if @virDirectory
      # oTmpDirs?.basedir? = @virDirectory.basedir

      if oTmpDirs?.isVirtual
        # console.log "============== is virtual":oTmpDirs
        if @entries.hasOwnProperty(name)
          # push a placeholder since this entry already exists but this helps
          # track the insertion index for the created views
          directories.push(name)
          ...
          expansionState = @expansionState.entries[name]
          expansionState ?= {}
          expansionState.isExpanded = oTmpDirs.isExpanded
          # expansionState.isExpanded = true
          # directories.push(new VirtualDirectory({name, fullPath, symlink:false, expansionState, @ignoredPatterns, @useSyncFS, virDirectory:oTmpDirs}))

          virDirectories.push(new Directory({name, fullPath:@path, symlink:false, expansionState, @ignoredPatterns, @useSyncFS, virDirectory:oTmpDirs}))
      else
        ...
        if stat.isDirectory?()
          if @entries.hasOwnProperty(name)
            ...
            unless !( oNewVirDirs and (_.size(oNewVirDirs) > 0))
              if oTmpDirs?.directories
                for k, v of oNewVirDirs
                  oTmpDirs.directories[k] = v
              else
                oTmpDirs.directories = oNewVirDirs

            expansionState = @expansionState.entries[name]
            expansionState ?= {}
            expansionState.isExpanded = oTmpDirs.isExpanded
            # expansionState.isExpanded = true
            directories.push(new Directory({name, fullPath, symlink, expansionState, @ignoredPatterns, @useSyncFS, stats: statFlat, virDirectory:oTmpDirs}))
        else if stat.isFile?()
          if @entries.hasOwnProperty(name)
            # push a placeholder since this entry already exists but this helps
            # track the insertion index for the created views
    ...
    @sortEntries(virDirectories).concat(@sortEntries(directories.concat(files)))
